...
class ModbusRTU: 
    ...
    def __init__(self, slave : int = 0x01, baudrate : int  = 19200, parity : int = 2, stopbit : int = 1, txenpin : machine.Pin = None, num_regs : int = 32  ): 
        ...
        self.led_builtin = machine.Pin( 25, machine.Pin.OUT ) 
    ...
    def recv(self): 
        while True:
            ...
            while sys.stdin in select.select( [sys.stdin], [sys.stdout], [sys.stderr], 0 )[0]:  
                self.msg_received += sys.stdin.buffer.read(1) 
                self.qnt_received += 1
                self.last_received = time.ticks_ms()
                time.sleep(0.01)
    ...
    def validate_request( self, BUFFER_IN : bytes ):
        ...
        # Checagem do FC 
        for function_code in self.FUNCTIONS_CODE.values():
        ...
        if BUFFER_IN[FC] == self.FUNCTIONS_CODE['FC_WRITE_SINGLE_REG']:
            sys.stdout.write( "WRITE SINGLE REG OK" )
        ...
        # ELSE tudo OK
        sys.stdout.write( "EXECUTANDO A AÇÂO" )
    ...
    def read_holding_registers(self, BUFFER ):
        ...
        starting_reg = int.from_bytes( BUFFER_CONV[REG_H:REG_L+1] , 'big')
        qnt_regs = int.from_bytes( BUFFER_CONV[QNT_H:QNT_L+1], 'big' )
        print( starting_reg, qnt_regs)
        
        if not( 1 <= qnt_regs <= 125 ):
           return self.EXECPTIONS['ILLEGAL_DATA_VALUE'] 
        else:
            PDU = bytearray( )
            PDU.append( BUFFER_CONV[ID] )    # FUNCTION CODE 0x03
