...
def serial_write_message(sender, data, user ):
    ...
    if   user == 'INITCM':
        user   = 'INITCM'.encode()
        user  += struct.pack('ff', dpg.get_value(46_11_43)[0], dpg.get_value(46_11_43)[1] )
    elif user == 'INITCP':
        user   = 'INITCM'.encode()
    ...
    ## Datetime Params 
        ...
        user  = "INITH".encode() 
        date  = [ int(val) for val in dpg.get_value( 46_11_1 )[:3] ] 
        hour  = [ int(val) for val in dpg.get_value( 46_11_2 )[:3] ]  
        if date[0] > 2000:  date[0] -= 2000
        if date[1] > 12:    return 'months out of range'       
        if date[2] > 31:    return 'days out of range'
        if hour[2] > 60:    return 'seconds out of range'
        if hour[1] > 60:    return 'minutes out of range'
        if hour[0] > 23:    return 'hours out of range'
        print(date, hour )
...
def serial_write_manual_message( sender, data, user ) : 
    if user == 'MSender':
        data = dpg.get_value(46_2_22)
    else: 
        data = ''

    if data == 'reset' or data == 'Reset':
        user = chr(3)  
    elif data == 'main': 
        user = '%Run -c $EDITOR_CONTENT'
    else: 
        user = data
    ...
    try: 
        if   type( user ) == str: 
            user = user.encode()  
        COMP.write( user )
        print( "Dentro de connections.serial.manualSender: Sender {}  data {}  user {}".format(sender, data, user ))
    except SerialException as e:
        print( e )
