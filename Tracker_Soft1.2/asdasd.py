...
class myModbusCommunication:
    ...
    # DESATIVAR O IRQ DO PINO rx 
    def deactive_irq(self):
        pass
    

    # ATIVAR O IRQ DO PINO rx
    def active_irq(self):
        pass
        ...
        while self.myUART.any():
            ...
            # IDENTIFICA AS FUNÇÕES DISPONÍVEIS 
                ...
                # COILS 
                    ...
                    if self.DEBUG:  print( 'FC = 1 : READ_COILS_REGISTER' )
            ...
            # VERIFICA A QUANTIDADE DE REGISTRADORES PARA LER 
                ...
                self._set_high_reg() 
                if self.FUNCTION_CODE == self.WRITE_COIL_REGISTERS:         self.STATE = self.N_REGISTERS
                elif self.FUNCTION_CODE == self.WRITE_HOLDING_REGISTERS:    self.STATE = self.N_REGISTERS
                else:                                                       self.STATE = self.CHK
            #-------------------------------------------------------
            ...
            # FUNCTION CODE - PRESET_MULTIPLE_REGISTERS 
                ...
                if self.NUM_BYTES > 0: 
                    ...
                    if self.DEBUG: print('DATA: ', self.DATA_BYTES )
                    self.STATE = self.CHK 
            #-------------------------------------------------------
            ...
            if self.STATE == self.RESOLVE:
                ...
                # READ_COILS_REGISTER 
                if  self.FUNCTION_CODE == self.READ_COILS_REGISTER:     
                    # QUERY 
                    ...
                    NUM_BYTES = math.ceil(self.QNT_REG/8)

                    # ler os registradores 
                    DATA_REG = 0
                    for num, byte in enumerate( self.COILS.STACK[ ADDR_REG : ADDR_REG+self.QNT_REG ]):
                        DATA_REG += 1 << num if byte else 0 
                    DATA_REG = struct.pack( 'B'*NUM_BYTES, DATA_REG )
                    # frame de dados 
                    DATA = NUM_BYTES + DATA_REG
                ...
                # READ_DISCRETE_INPUT 
                    ...
                    # ler os registradores 
                    DATA_REG = 0
                    for num, byte in enumerate( self.DISCRETES.STACK[ ADDR_REG : ADDR_REG+self.QNT_REG ]):
                        DATA_REG += 1 << num if byte else 0 
                    DATA_REG = struct.pack( 'B'*NUM_BYTES, DATA_REG )
                    # frame de dados 
                ...
                # READ_HOLDING_REGISTER
                    ...
                    ADDR_REG  = self.ADDR_REG
                    NUM_BYTES = struct.pack('B', 2*self.QNT_REG) 

                    # ler os registradores 
                    DATA_REG = b''
                    for byte in self.HOLDINGS.STACK[ ADDR_REG : ADDR_REG+self.QNT_REG ]:
                        DATA_REG += struct.pack( '>H', int(byte*100) )
                ...
                # READ_INPUT_REGISTER 
                    ...
                    # 11 04 02 000A F8F4
                    ADDR_REG  = self.ADDR_REG
                    NUM_BYTES = struct.pack('B', 4*self.QNT_REG) 

                    # ler os registradores 
                    DATA_REG = 0
                    for byte in self.INPUTS.STACK[ ADDR_REG : ADDR_REG+self.QNT_REG ]:
                        DATA_REG += struct.pack( 'f', byte )
                    # frame de dados 
                    DATA = NUM_BYTES + DATA_REG
                ...
                #  COMUM DESDE QUE MANTENHA A ESTRUTURA 
                self.response( self.FUNCTION_CODE, DATA )
                self.STATE = self.SLAVE
                self.MSG_GOT = b'' 
            # -----------------------------------------------
    ...
    # Verifica se um CHK de 16bits é compatível com a mensagem recebida 
    @property
    def check_crc16( self ):
        ...
        if self.DEBUG : print( 'CRC calculado: {}\nCRC recebido: {}'.format( crc, got ))
        return crc == got

    # Gera um check sum de 16 bits dado um quadro de bytes 
    def get_crc16(self, data ):
        crc = 0xFFFF
        for char in data:
            crc = (crc >> 8) ^ self.CRC16_TABLE[((crc) ^ char) & 0xFF]
        crc = struct.pack('<H', crc) 
        if self.DEBUG: print( 'CRC P/ envio: {}'.format( crc ) )
        return crc 
