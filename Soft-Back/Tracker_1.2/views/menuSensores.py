...
def init_sensores( windows : dict ):       
    ...
    with dpg.window(id=53_0, no_title_bar=True, no_move=True, no_resize=True, no_close=True, no_collapse=True) as WinPltMx:
        windows['Sensores'].append(WinPltMx)
        dpg.add_text('Log dos sensores (Rasp)')
        dpg.add_text( tag = 53_1, default_value = 'DESCONECTADO!' )

    with dpg.window(id = 54_0, no_close=True, no_move=True, no_resize=True, no_title_bar=True, no_collapse=True ) as Comandos: 
        windows['Sensores'].append(Comandos)
        with dpg.child_window( tag = 54_10 ): 
            with dpg.group( horizontal = True ):    
                dpg.add_text('Diagnósticos:')   
                dpg.add_button( label = 'Solicitar diagnósticos', tag = 5_4_1, callback = serial_request_diagnosis ) 
            dpg.add_text( '', tag = 54_2 )
            row_names = ["\tGir", '\tPV', "\tError", '\tConf', "\tD", "\tI", "\tP", "\tEle", '\tPV', "\tError", '\tConf', "\tD", "\tI", "\tP" ]
            with dpg.table( tag = 54_30, header_row = True, label = 'Diagnosis uC Pico'):
        ...
        with dpg.child_window( tag = 54_20 ):  
            dpg.add_text( 'Informações de registro' )
