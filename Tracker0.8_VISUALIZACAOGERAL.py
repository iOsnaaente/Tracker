...
with window( label = 'Atuação'          , id = 2_2_0, no_move = True, no_resize = True, no_collapse = True, no_close = True ) as Atuacao_VG:
    ...
    def att_draw_gir():
        ...
        configure_item( 2_2_1_1_2, p1     = [ r_gir*cos(math.radians(MG_Angle -math.pi/2))+w_gir, r_gir*sin(math.radians(MG_Angle -math.pi/2)) +h_gir ] , p2     = center_gir               ) 
        configure_item( 2_2_1_1_3, p1     = [ r_gir*cos(math.radians(AZI_Angle-math.pi/2))+w_gir, r_gir*sin(math.radians(AZI_Angle-math.pi/2))+h_gir ] , p2     = center_gir               ) 
    ...
    def att_draw_ele() :
        ...
        center_ele   = [ w_ele - r_ele/2, r_ele+10]
        ...
        else:              configure_item( 2_2_2_1_3, p1 = [ r_ele*cos(math.radians(ALT_Angle-math.pi/2))+center_ele[0], r_ele*sin(math.radians(ALT_Angle-math.pi/2))+center_ele[1] ] , p2 = center_ele )
        configure_item( 2_2_2_1_2, p1 = [ r_ele*cos(math.radians(ME_Angle-math.pi/2))+center_ele[0], r_ele*sin(math.radians(ME_Angle-math.pi/2)) +center_ele[1] ] , p2 = center_ele )
...
with window( label = 'Painel de log'       , id = 2_3_0, no_move = True, no_resize = True, no_collapse = True, no_close = True, no_title_bar = True ) as Painel_log_VG:
    ...
    add_drag_floatx( label = 'Dia automatico'  , id = 2_3_1, size = 3, format = '%4.0f', min_value = 1, max_value = 3000, speed=1, no_input= True)
    add_spacing(count = 1)
    ...
    add_spacing(count = 1 )
    add_input_floatx( label = 'Dia arbitrario' , id = 2_3_6, size = 3, default_value = [2020, 12, 25], format='%.0f', min_value = 1, max_value = 3000, enabled = False )
    add_spacing(count = 1 )
    add_input_floatx( label = 'Hora arbitraria', id = 2_3_7, size = 3, default_value = [20, 30, 10]  , format='%.0f', min_value = 1, max_value = 60, enabled = False )
    add_spacing(count = 1 )
