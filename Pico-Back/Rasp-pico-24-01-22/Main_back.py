...
try:
    isc0   = SoftI2C ( freq = 100000, sda = Pin( SDA_DS  ), scl = Pin( SCL_DS  ) ) 
...
try: 
    if 104 in isc0.scan():
        Time = DS3231( isc0  )
        #Time.set_time( 22, 1, 20, 15, 49, 50 )
    else:
        print( "I2C com erro - DS3231 não conectado ") 
except:
    print( 'Breka Time')
    machine.soft_reset()
...
PID_GIR = mp.PID( PV = AZIMUTE , Kp = 0.5, Kd = 0.1, Ki = 0.5 )
PID_ELE = mp.PID( PV = ALTITUDE, Kp = 0.5, Kd = 0.1, Ki = 0.5 )
...
while True:
    ...
    if STATE == AUTOMATIC: 
        ...
        PID_ELE.set_PV( ALTITUDE if ALTITUDE > 0 else 10 )
