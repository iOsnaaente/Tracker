...
while True:
    ...
    # INICIA O MODO DE OPEÇÃO ACELERADO ( DEMONSTRAÇÃO )
        ...
        TIME = timanager.up_fake_time( up = False ) 
        timanager.update( TIME )
        counter_demo += 1
        if counter_demo%10 == 0:
            timanager.print() 
    ...
    while sys.stdin in select.select( [sys.stdin], [sys.stdout], [sys.stderr], 0 )[0]:  
        ...
        if count == 4:
            ...
            if   BYTE_ID == b'H':
                ...
                confirm = sys.stdin.buffer.read(1)
                if confirm == b'O':
                    print('Recebido: {} para iniciar o modo Levers'.format( BYTE_ID))
                    STATE = MANUAL_CONTROLING
                    time_delay = 0.001
